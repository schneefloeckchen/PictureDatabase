...
//   Check loadById and correct model
  ...
  /**
   * wie oben, nur anlegen der Cameras per Stream interface
   */
  @Test
  protected void TestCreateMultipleCamera2() {
    String[][] cameraData = {
      {"FinePix 1200", "Fuij"},
      {"M70", "Cannon"},
      {"M75", "Cannon"},
      ...
      {"FX204", "Nikon"},
      {"FX205", "Nikon"}};
    Arrays.stream(cameraData).forEach(row -> {
      assertEquals(2, row.length, "Zeilenlaenge");
      Camera camera = new Camera(row[0], row[1]);
      camera.update();
    } );
    assertEquals (cameraData.length, util.countElementsInTable("CAMERA"),
            "Number of created cameras");
  }
